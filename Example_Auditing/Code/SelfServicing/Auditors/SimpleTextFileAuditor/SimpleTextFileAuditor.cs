...
namespace SD.LLBLGen.Pro.Examples.Auditing.Auditors
{
    ...
    /// to a text file using LLBLGenPro v2.5 Auditor Dependency Injection.
    /// 
    ...
    /// text file in a web-based scenario. This example isn't a must-be approach, just an illustration
	/// how this could work. This example uses a winforms application, which is single-threaded, single
	/// user by nature and therefore doesn't run into threading issues.
    /// </summary>
    /// <remarks>
    ...
    public class SimpleTextFileAuditor : AuditorBase
    {
        ...
        private void PersistAuditInfoThreadSafe(StringWriter sw)
        {
            ...
            // avoid multiple threads to open the file at the same time
            lock (this)
            {
        ...
        public override void AuditDeleteOfEntity(IEntityCore entity)
        {
            ...
            StringWriter auditInfo = ConstructAuditRecord(entity.LLBLGenProEntityName,
                AuditType.LoadOfExistingEntity, GetPrimaryKeyInfoFromEntity(entity));
        ...
        {
            /// Doesn't use AddAuditRecordToStream method because when a process requests an entity (load)
            /// it maybe never saves this entity again, however we want to log the load. So we would persist 
            /// right here.
            /// 
            ...
            /// Uncomment below code to test this functionallity.
            ...
            IRelationCollection relations, int numberOfEntitiesDeleted)
        {
            ...
            // Doesn't use AddAuditRecordToStream method because there is a directly delete, so
            // TransactionCommited would no be called.
            ...
            // construct audit info record
            StringWriter auditInfo = ConstructAuditRecord(typeOfEntity.ToString(),
        ...
        public override void AuditEntityFieldGet(IEntityCore entity, int fieldIndex)
        {
			...
			StringWriter auditInfo = ConstructAuditRecord(entity.LLBLGenProEntityName,
				...
				string.Format("{0}. FieldGet: {1}",
					GetPrimaryKeyInfoFromEntity(entity), ((IEntity)entity).Fields[fieldIndex].Name));
        ...
        public override void AuditReferenceOfRelatedEntity(IEntityCore entity, IEntityCore relatedEntity, string mappedFieldName)
        {
            ...
            // construct audit info record
            StringWriter auditInfo = ConstructAuditRecord(entity.LLBLGenProEntityName,
                AuditType.ReferenceOfRelatedEntity,
	    ...
	    {
            // avoid to audit into AuditInfoEntity (this would create an overflow effect)
            if (entity is AuditInfoEntity)
            {
                return;
            }
