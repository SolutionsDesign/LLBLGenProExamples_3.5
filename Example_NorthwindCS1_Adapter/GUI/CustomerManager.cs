...
namespace Northwind.GUI
{
	...
	public class CustomerManager : System.Windows.Forms.Form
	{
		...
		private bool				_customerObjectIsBound;
		private ISortExpression		_orderSorter;
		private CurrencyManager		_ordersCManager;
		private List<EntityCollection<OrderDetailEntity>> _visitedOrderDetail;
		#endregion
		...
		public CustomerManager()
		{
			...
			_customerObjectIsBound=false;

			// block resizing beyond a minimum;
			this.MinimumSize = this.Size;

			// set ordersorter expression
			_orderSorter = new SortExpression(OrderFields.OrderId | SortOperator.Ascending);

			_ordersCManager=null;

			_visitedOrderDetail = new List<EntityCollection<OrderDetailEntity>>();
		}
		...
		private void BindCustomerToGui()
		{
			...
			// Bind the customer's propertychanged event so we'll get a signal something changed and we can enable the save changes button.
			_currentCustomer.PropertyChanged += new PropertyChangedEventHandler(_currentCustomer_PropertyChanged);
			_customerObjectIsBound = true;
		...
		private void BindOrders()
		{
			...
			for (int i = 0; i < _visitedOrderDetail.Count; i++)
			{
				_visitedOrderDetail[i].ListChanged-=new ListChangedEventHandler(CurrentOrderDetail_ListChanged);
			}
			_visitedOrderDetail.Clear();
			...
			if(_currentCustomer.Orders.Count>0)
			{
				...
				if(!_visitedOrderDetail.Contains( _currentCustomer.Orders[ordersDataGrid.CurrentRowIndex].OrderDetails))
				{
					// add it
					_visitedOrderDetail.Add(_currentCustomer.Orders[ordersDataGrid.CurrentRowIndex].OrderDetails);
					
					// add eventhandler
					OrderEntity currentSelectedOrder = _currentCustomer.Orders[ordersDataGrid.CurrentRowIndex];
					currentSelectedOrder.OrderDetails.ListChanged+=new ListChangedEventHandler(CurrentOrderDetail_ListChanged);
		...
		private void InitializeComponent()
		{
			...
			// panel1
			// 
			this.panel1.Controls.Add(this.refreshOrdersButton);
			...
			this.refreshOrdersButton.Enabled = false;
			this.refreshOrdersButton.Location = new System.Drawing.Point(435, 9);
			this.refreshOrdersButton.Name = "refreshOrdersButton";
			this.refreshOrdersButton.Size = new System.Drawing.Size(105, 23);
			this.refreshOrdersButton.TabIndex = 2;
		...
		private void _currentCustomer_PropertyChanged(object sender, PropertyChangedEventArgs e)
		{
		...
		private void saveCustomerButton_Click(object sender, System.EventArgs e)
		{
			...
			DataAccessAdapter adapter = new DataAccessAdapter();
			bool succeeded = adapter.SaveEntity(_currentCustomer, true);
		...
		private void saveOrdersButton_Click(object sender, System.EventArgs e)
		{
			...
			// save all orders which are changed to the persistent storage. 
			// we simply save the customer's Order collection, all contents of those orders are saved as well.
			DataAccessAdapter adapter = new DataAccessAdapter();
			int amountOrdersSaved = adapter.SaveEntityCollection(_currentCustomer.Orders, true, true);
			saveOrdersButton.Enabled=false;
		...
		private void refreshOrdersButton_Click(object sender, System.EventArgs e)
		{
			if(_currentCustomer.Orders.ContainsDirtyContents)
			{
				DialogResult result = MessageBox.Show(this, "You have pending changes. If you continue, you will lose those changes. Continue?", "Confirmation wanted.", MessageBoxButtons.YesNo, MessageBoxIcon.Warning, MessageBoxDefaultButton.Button2);
		...
		private void CurrentOrderDetail_ListChanged(object sender, ListChangedEventArgs e)
		{
		...
		private void removeOrderButton_Click(object sender, System.EventArgs e)
		{
			...
			OrderEntity currentOrder = (OrderEntity)_currentCustomer.Orders[ordersDataGrid.CurrentRowIndex];
